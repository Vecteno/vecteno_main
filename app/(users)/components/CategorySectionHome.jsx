...
const CategoriesSection = () => {
  ...
  // Update itemsPerView on resize - MODIFIED FOR MOBILE
  const updateItemsPerView = useCallback(() => {
    const w = window.innerWidth;
    if (w >= 1280) setItemsPerView(6);
    else if (w >= 1024) setItemsPerView(5);
    else if (w >= 768) setItemsPerView(3);
    else setItemsPerView(2); // Changed from 1 to 2 for mobile
  }, []);
  ...
  // Calculate translateX in % (slide by one group at a time)
  const translateXPercent = currentIndex * (100 / (categories.length / itemsPerView));
          ...
          >
            {categories.map((category) => (
              <a
                key={category.id}
                href={`/${category.slug}/`}
                className="flex-shrink-1 px-2 md:px-4" // Reduced padding on mobile
                style={{
                  width: `${100 / itemsPerView}%`, // Each item takes equal portion of the view
                }}
              >
                <div className="group block cursor-pointer rounded-lg overflow-hidden w-full shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 bg-white">
                  <div
...
//                 key={category.id}
//                 href={`/${category.slug}/`}
//                 className="flex-shrink-1 px-3 md:px-4"
//                 style={{
